...
xmlNanoFTPScanURL(void *ctx, const char *URL) {
    ...
    buf[index] = 0;
	while (*cur != 0) {
		int cur_test = cur[0] == ':' && cur[3] == ' ';
		#ifdef CUSTOM_1
		cur_test = cur_test && (cur[1] == '/' || cur[4] == ' '); 
		#else
		cur_test = cur_test && (cur[2] == '0' || cur[5] == ' '));
		#endif
	   if (cur_test) {
			buf[index] = 0;
			index = 0;
    ...
    while (1) {
        ...
        int cur_test = (*cur == '/') || (*cur == '1');
		#ifdef CUSTOM_1
		cur_test = cur_test || (*cur == 0);
		#elif defined CUSTOM_2
		cur_test = cur_test | (*cur == ' ');
		#endif
		if (cur_test) {
	    buf[index] = 0;
...
xmlNanoFTPConnect(void *ctx) {
	...
	switch (proxyType) {
	    ...
	    case 1:
		/* Using SITE command */
/* ToDO Adicionar a função fnprintf recebendo os paramêtros (buf, "SITE %s\r\n", sizeof(buf), ctxt->hostname)*/		
#ifndef HAVE_SNPRINTF
...
xmlNanoFTPGetConnection(void *ctx) {
	...
	/* ToDo: para a implementação do WIN_64 a função sprintf2 e snprintf irão receber variáveis diferentes na chamada.
A variável após adp[0] & 0xff deverá ser trocada por portp[2] & 0xff
 */
#ifdef WIN_32
	#define PARAM_1 adp[1] & 0xff
	#define PARAM_2 portp[1] & 0xff
#else
	#define PARAM_1 portp[1] & 0xff
	#define PARAM_2 adp[1] & 0xff
#endif
#ifndef HAVE_SNPRINTF
 	len = sprintf2(buf, "PORT %d,%d,%d,%d,%d,%d\r\n",
	       adp[0] & 0xff, PARAM_1, adp[2] & 0xff, adp[3] & 0xff,
	       portp[0] & 0xff, PARAM_2);
#else /* HAVE_SNPRINTF */
...
/* ToDO Adicionar a função fnprintf recebendo os paramêtros (buf, "LIST -L\r\n", sizeof(buf))*/
#ifndef HAVE_SNPRINTF
...
xmlNanoFTPGetSocket(void *ctx, const char *filename) {
    ...
    if (filename == NULL)
/* ToDO Adicionar a função fnprintf recebendo os paramêtros (buf, "RETR %s\r\n", sizeof(buf), ctxt->path)*/		
#ifndef HAVE_SNPRINTF
