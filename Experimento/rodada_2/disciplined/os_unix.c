...
static void set_x11_title __ARGS((char *));
static void set_x11_icon __ARGS((char *));
# endif
#endif
...
static int	unix_expandpath __ARGS((garray_T *gap, char *path, int wildoff, int flags));
# if defined(MACOS_X) && defined(FEAT_MBYTE)
extern char	*mac_precompose_path __ARGS((char *decompPath, size_t decompLen, size_t *precompLen));
# endif
#endif
...
#ifndef __EMX__
static char	*extra_shell_arg = NULL;
...
static struct signalinfo
{
    ...
    char    *name;	/* Signal name (not char!). */
    ...
    char    **argv;
{
	...
	exe_name = vim_strsave((char *)argv[0]);
#endif
    if (isatty(1))
	return OK;
    return FAIL;
    ...
    struct timeval *tvp;	/* must contain start time */
{
    ...
    smsg((char *)_("Opening the X display took %ld msec"),
    ...
    Display	*dpy;
{
	...
	verb_msg((char *)_("Testing the X display failed"));
...
#define XD_XTERM 3	/* x11_display used from xterm_dpy */
	...
	if (p_verbose > 0 && sig_alarm_called)
	    verb_msg((char *)_("Opening the X display timed out"));
#endif
	if (x11_display != NULL)
	{
# if defined(HAVE_GETTIMEOFDAY) && defined(HAVE_SYS_TIME_H)
...
#endif
		    if (get_title)
			oldtitle = vim_strsave((char *)text_prop.value);
		    else
			oldicon = vim_strsave((char *)text_prop.value);
#ifdef FEAT_XFONTSET
		}
		else
		{
		    ...
		    {
			if (get_title)
			    oldtitle = vim_strsave((char *) cl[0]);
			else
			    oldicon = vim_strsave((char *) cl[0]);
			XFreeStringList(cl);
		    }
		    else
		    {
    ...
    char  *name;
{
    ...
    return ((STRNICMP(name, "vt", 2) == 0
		&& vim_strchr((char *)"12345", name[2]) != NULL)
    ...
    int		len;
{
    ...
    {
	vim_strncpy(s, (char *)pw->pw_name, len - 1);
	return OK;
    }
#endif
    ...
    int	    len;
{
	...
	*s = NUL;
	...
	vim_strncpy(s, (char *)vutsname.nodename, len - 1);
}
...
static int executable_file __ARGS((char *name));
...
#endif
	buf[1] = NUL;
	add_termcode((char *)"kb", buf, FALSE);

	/*
	 * If <BS> and <DEL> are now the same, redefine <DEL>.
	 */
	p = find_termcode((char *)"kD");
...
# ifdef FEAT_MOUSE_DEC
	...
	if (on)	/* enable mouse events */
	    out_str_nf((char *)"\033[1;2'z\033[1;3'{");
	else	/* disable mouse events */
	    out_str_nf((char *)"\033['z");
	ison = on;
    }
# endif
...
# ifdef FEAT_MOUSE_JSB
	...
	if (on)
	{
	    ...
	    out_str_nf((char *)IF_EB("\033[0~ZwLMRK1Q\033\\",
					 ESC_STR "[0~ZwLMRK1Q" ESC_STR "\\"));
#else
	    out_str_nf((char *)IF_EB("\033[0~ZwLMRK+1Q\033\\",
	...
	{
	    out_str_nf((char *)IF_EB("\033[0~ZwQ\033\\",
...
#  endif
    ...
    {
	set_mouse_termcode(KS_MOUSE, (char *)(term_is_8bit(T_NAME)
...
#  endif
	...
	set_mouse_termcode(KS_MOUSE, (char *)IF_EB("\033MG", ESC_STR "MG"));
# endif
...
#  endif
	...
	set_mouse_termcode(KS_JSBTERM_MOUSE,
			       (char *)IF_EB("\033[0~zw", ESC_STR "[0~zw"));
    else
...
#  endif
	...
	set_mouse_termcode(KS_DEC_MOUSE, (char *)(term_is_8bit(T_NAME)
    ...
    garray_T	*gap;
{
    ...
    if (gap->ga_len > 0
	    && !curbuf->b_p_bin
	    && ((char *)gap->ga_data)[gap->ga_len - 1] == CAR)
	--gap->ga_len;
    ga_append(gap, NUL);
    ...
    int		options;	/* SHELL_*, see vim.h */
{
    ...
    int	    x;
# ifndef __EMX__
    char  *newcmd;   /* only needed for unix */
# else
    /*
    ...
    if (((old = (char *)getenv("EMXSHELL")) == NULL) || STRCMP(old, p_sh))
    {
...
# endif
		char	    ta_buf[BUFLEN + 1];	/* TypeAHead */
    ...
    long	msec;
{
	...
	mch_write((char *)IF_EB("\033[1'|", ESC_STR "[1'|"), 5);
    }
#endif
...
# ifdef USE_XSMP
	if (xsmp_idx >= 0 && (fds[xsmp_idx].revents & (POLLIN | POLLHUP)))
	{
	    ...
	    {
		if (p_verbose > 0)
		    verb_msg((char *)_("XSMP lost ICE connection"));
		xsmp_close();
	    }
	    if (--ret == 0)
    ...
    *s = c;
	...
	for (;;)
	{
	    ...
	    if ((dp->d_name[0] != '.' || starts_with_dot)
		    && vim_regexec(&regmatch, (char *)dp->d_name, (colnr_T)0))
	    {
		STRCPY(s, dp->d_name);
		len = STRLEN(buf);
...
# define EXPL_ALLOC_INC	16
    ...
    *file = (char **)alloc(sizeof(char **) * files_alloced);
    if (*file == NULL)
	return FAIL;
	...
	if (has_wildcard)   /* yes, so expand them */
	    expl_files = (char **)_fnexplode(buf);
	...
	{   /* simply save the current contents of *buf */
	    expl_files = (char **)alloc(sizeof(char **) * 2);
	    if (expl_files != NULL)
	    {
	...
	if (expl_files)
	{
		...
		if (--files_free == 0)
		{
		    ...
		    *file = (char **)vim_realloc(*file,
					   sizeof(char **) * files_alloced);
		    if (*file == NULL)
		    {
			EMSG(_(e_outofmem));
			*num_file = 0;
...
# endif
	...
	++len;				/* add space */
	for (j = 0; pat[i][j] != NUL; )
	    if (vim_strchr((char *)" '", pat[i][j]) != NULL)
	    {
		len += 2;		/* add two quotes */
		while (pat[i][j] != NUL
			&& vim_strchr((char *)" '", pat[i][j]) != NULL)
		{
		    ++len;
		    ++j;
		}
	...
	*p = ' ';				/* remove last backtick */
	...
	for (i = 0; i < num_pat; ++i)
	{
	    ...
	    *p++ = ' ';
		...
		else if (!intick && vim_strchr((char *)" '",
							   pat[i][j]) != NULL)
		{
	...
	extra_shell_arg = (char *)"-G";   /* Use zsh NULL_GLOB option */
     ...
     */
    else if (shell_style == STYLE_GLOB)
	extra_shell_arg = (char *)"-f";	/* Use csh fast option */

    /*
     * execute the shell command
     */
    i = call_shell(command, 1);
...
typedef char * (*STRPROCSTR)__ARGS((char *));
typedef char * (*INTPROCSTR)__ARGS((int));
typedef int (*STRPROCINT)__ARGS((char *));
...
mch_libcall(libname, funcname, argstring, argint, string_result, number_result)
    char	*libname;
    char	*funcname;
    char	*argstring;	/* NULL when using a argint */
    int		argint;
    char	**string_result;/* NULL when using number_result */
    int		*number_result;
{
...
# endif
	...
	{
	    if (p_verbose > 0)
		verb_msg((char *)_("Opening the X display failed"));
	    return;
	}
...
do_xterm_trace()
{
    ...
    char		*strp;
    long		got_hints;
    static char	*mouse_code;
    static char	mouse_name[2] = {KS_MOUSE, KE_FILLER};
    ...
    Bool	fast;
{
    ...
    if (p_verbose > 0)
	verb_msg((char *)_("XSMP handling save-yourself request"));
...
xsmp_init(void)
{
    ...
    if (p_verbose > 0)
	verb_msg((char *)_("XSMP opening connection"));
    ...
    {
	if (p_verbose > 0)
	    verb_msg((char *)_("XSMP ICE connection watch failed"));
	return;
    }
